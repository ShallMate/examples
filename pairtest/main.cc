...
int main() {
    ...
    auto pairing_group = yacl::crypto::PairingGroupFactory::Instance().Create(library_name);
    ...
    if (pairing_group) {
        ...
        start = std::chrono::high_resolution_clock::now();
        auto right = pairing_group->GetGroupT()->Mul(pairing_group->Pairing(p1, q), pairing_group->Pairing(p2, q));
        end = std::chrono::high_resolution_clock::now();
