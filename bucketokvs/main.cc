...
// 自定义比较函数，按 hashes 排序
bool compare_triples(const std::tuple<Triple, uint128_t, uint128_t>& a, const std::tuple<Triple, uint128_t, uint128_t>& b) {
    return std::get<0>(a) < std::get<0>(b);
}

void SortHashesAndSyncKeysValues(std::vector<Triple>& hashes, std::vector<uint128_t>& keys, std::vector<uint128_t>& values) {
...
void GetHashes(std::vector<uint128_t> data,uint32_t cuckoolen,std::vector<Triple>& hashes) {
