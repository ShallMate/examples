...
int main() {
  ...
  // case by a std::stringstream, but some other C++ stream could be used as
  // well.
  ...
  // Create the Sender's database (we are setting up an unlabeled SenderDB
  // here). The SenderDB should typically live in a std::shared_ptr.
  shared_ptr<sender::SenderDB> sender_db =
      make_shared<sender::SenderDB>(params);
  ...
  // We need to convert the strings to Item objects
  vector<Item> receiver_items(raw_receiver_items.begin(),
                              raw_receiver_items.end());
  ...
  oprf::OPRFReceiver oprf_receiver =
      receiver::Receiver::CreateOPRFReceiver(receiver_items);
  ...
  Request received_request =
      channel.receive_operation(sender_db->get_seal_context());
  OPRFRequest received_oprf_request =
      to_oprf_request(std::move(received_request));
  ...
  auto receiver_oprf_items =
      receiver::Receiver::ExtractHashes(oprf_response, oprf_receiver);
  ...
  while ((result_part_count--) != 0U) {
    ResultPart result_part =
        channel.receive_result(receiver.get_seal_context());
  ...
  vector<string> raw_sender_items_add{"Amir", "Danny"};
  vector<Item> sender_new_items(raw_sender_items_add.begin(),
                                raw_sender_items_add.end());
  // Insert the items in the SenderDB
  ...
  */
  ...
  vector<string> raw_receiver_new{"Bob", "AA"};
