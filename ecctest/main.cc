...
Triple sort_triple(Triple t) {
  ...
  return std::make_tuple(arr[0], arr[1],
                         arr[2]);  // 将排序后的数组重新生成 tuple
}

// 假设 yacl::Buffer 提供 .data() 和 .size() 方法
void SerializeVector(const std::vector<yacl::Buffer>& XS,
  ...
  outfile.write(reinterpret_cast<const char*>(&vector_size),
                sizeof(vector_size));

  // 遍历并序列化每个 yacl::Buffer
  for (const auto& buffer : XS) {
    size_t buffer_size = buffer.size();  // 假设 buffer.size() 返回字节大小
    outfile.write(reinterpret_cast<const char*>(&buffer_size),
                  sizeof(buffer_size));  // 写入 buffer 大小
    outfile.write(reinterpret_cast<const char*>(buffer.data()),
                  buffer_size);  // 写入 buffer 数据
  }
...
void Insert(std::vector<yacl::Buffer> data, uint32_t cuckoolen) {
  ...
  for (size_t i = 0; i < data.size(); ++i) {
    ...
    if (unique_triples.find(new_triple) != unique_triples.end()) {
      std::cout << "发现重复的三元组: (" << a << ", " << b << ", " << c << ")"
                << std::endl;
      break;
    } else {
      unique_triples.insert(new_triple);
      std::cout << "生成三元组: (" << a << ", " << b << ", " << c << ")"
                << std::endl;
    }
  }
}
