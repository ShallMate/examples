...
std::vector<uint128_t> GetIntersection(std::vector<uint32_t> z,std::vector<uint128_t> y,uint32_t z_size) {
...
std::vector<uint32_t> ConvertToUint32Vector(const std::vector<uint8_t>& input,uint32_t z_size) {
...
void EcdhReceiver::BuffertoPoints(absl::Span<yc::EcPoint> in, absl::Span<std::uint8_t> buffer){
  yacl::parallel_for(0, in.size(), [&](size_t begin, size_t end) {
  for (size_t idx = begin; idx < end; ++idx) {
    uint64_t offset = idx*32;
...
std::vector<uint128_t> EcdhReceiver::EcdhPsiRecv(const std::shared_ptr<yacl::link::Context>& ctx,std::vector<uint128_t>& y){
