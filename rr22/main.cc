...
struct Uint128Hash {
  size_t operator()(const uint128_t& key) const {
    return std::hash<uint64_t>()(static_cast<uint64_t>(key)) ^
           std::hash<uint64_t>()(static_cast<uint64_t>(key >> 64));
  }
};
...
std::vector<uint128_t> RR22PsiRecv(
  ...
  unordered_map<uint128_t, uint128_t, Uint128Hash> receiver_to_elem_map;

  for (size_t i = 0; i < receivermasks.size(); ++i) {
    receiver_to_elem_map[receivermasks[i]] = elem_hashes[i];
  }

  std::vector<uint128_t> intersection_elements;
  for (const auto& mask : sendermasks) {
    auto it = receiver_to_elem_map.find(mask);
    if (it != receiver_to_elem_map.end()) {
      intersection_elements.push_back(it->second);
    }
  }
