...
std::vector<uint128_t> APSI::APsiRun(std::vector<uint128_t>& items) {
  ...
  apsi::Request request = apsi::receiver::Receiver::CreateOPRFRequest(oprf_receiver);
  ...
  apsi::sender::Sender::RunOPRF(received_oprf_request, sender_db->get_oprf_key(),
                          *channel_);
  ...
  apsi::QueryRequest received_query_request = apsi::to_query_request(received_request);
